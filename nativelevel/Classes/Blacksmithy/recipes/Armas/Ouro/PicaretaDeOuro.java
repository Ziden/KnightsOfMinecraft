 ...
 *
 */
public class PicaretaDeOuro extends CustomCrafting {

    public PicaretaDeOuro() {
        super("Picaretas de Ouro Aprimoradas", "Crie picaretas de ouro aprimoradas");
    }
    ...
    public ItemStack aplica(ItemStack ss) {
        ...
        ss.addEnchantment(Enchantment.DURABILITY, 2);
        return ss;
    }

    @Override
    public ItemStack aplicaNoCraftNormal(ItemStack ss) {
        if(ss.getType()!=Material.GOLD_PICKAXE) 
            return ss;
        double dano = WeaponDamage.getDamage(ss);
        dano -= 1;
